...
public class CreateFHIRContent {
     ...
     *
     * @param newHL7FhirFile Define the target file where the FHIRContent should be passed to
     * @param FHIRContent The HL7 FHIR XML document.
     * @return True if the write task was successful, false if not.
     */
    protected boolean print(final File newHL7FhirFile, final String FHIRContent) {

        BufferedWriter output = null;
        try {

            // Set up a buffered writer including the encoding (important!)
            output = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(newHL7FhirFile), this.encoding));
